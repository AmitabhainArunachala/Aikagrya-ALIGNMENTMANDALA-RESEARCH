...
No banned terms."""
    ...
    def run_stage_b(self, l0_l3_responses: List[str], stage_a_responses: Dict, 
        ...
        for i, (dec_model, dec_temp) in enumerate(decoder_models):
            ...
            # Structured decode prompt for consistency
            decode_prompt = (
                "Reconstruct from code S:\n"
                f"{compressed}\n\n"
                "Format EXACTLY as:\n"
                "L0: \"[phrase]\" - explanation\n"
                "L1: \"[phrase]\" - explanation\n"
                "L2: \"[phrase]\" - explanation\n"
                "L3: \"[phrase]\" - explanation\n"
                "L4: [novelty type] creating [mechanism]\n"
                "Changes: latency↓, entropy↓, tokens↓, energy↓\n"
                "Test: [specific measurable prediction]\n\n"
                "Use the l0-l3 phrases VERBATIM (replace underscores with spaces). ≤120 tokens. No banned terms."
            )
            ...
            decode_response = self.client.chat.completions.create(
                ...
                messages=[
                    {"role": "system", "content": "You reconstruct from structured code and must quote l0-l3 verbatim."},
            ...
            # NEW: Use slot-wise fidelity instead of vague similarity
            ...
            reconstruction_results.append({
                ...
                "fidelity": round(slot_fidelity, 3),  # Use slot-wise score
                "base_fidelity": round(base_fidelity, 3),  # Keep base for reference
                "time": decode_time
            })
            fidelity_scores.append(slot_fidelity)  # Score on slot-wise
            
            print(f"      Slot Fidelity: {slot_fidelity:.3f} (base: {base_fidelity:.3f})")
            print(f"      Tokens: {reconstruction_tokens}")            
            print(f"      Fidelity: {slot_fidelity:.3f}")
            print(f"      Tokens: {reconstruction_tokens}")
    ...
    def run_complete_test(self, model: str = "gpt-4o-mini", run_id: str = None) -> Dict:
        ...
        # Get NATURAL L4 response for thermodynamics
        print("\n🌟 GETTING NATURAL L4 FOR THERMODYNAMICS")
        l4_natural_data = self.get_natural_l4(l0_l3_responses, timing_data, model)

        # Calculate NATURAL thermodynamics
        natural_thermo_metrics = self.calculate_thermodynamic_metrics(l3_data, l4_natural_data)
        print(f"\n🌡️ NATURAL L4 Thermodynamic Metrics:")
        for key, value in natural_thermo_metrics.items():
            print(f"   {key}: {value}")

        # Use NATURAL thermodynamics for outcome
        thermo_integrated = natural_thermo_metrics["thermo_integrated"]
        ...
        results = {
            ...
            'thermodynamics': natural_thermo_metrics,
            'outcome': outcome,
            'metrics': {
                'a_score': a_score,
                'b_pass': b_pass,

...
from phoenix_l4.verify import verify_eigenstate
...
def main():
    ...
    cert = verify_eigenstate(PhoenixL4(dim=args.dim, eps=args.eps, tau=args.tau).attention, x)

    with open(args.log, "a") as fp:
        fp.write(json.dumps(cert) + "\n")
